...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	ВТ КАК ВТ
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ПартииТоваров.Остатки(
		|				&МоментВремени,
		|				Товар В
		|					(ВЫБРАТЬ
		|						ВТ.Товар КАК Товар
	...
	Пока ВыборкаТовар.Следующий() Цикл
		...
		Пока ВыборкаПартия.Следующий() И КоличествоСписать <> 0 Цикл
			...
			Количество = Мин(КоличествоСписать, ВыборкаПартия.КоличествоОстаток);
			Если КоличествоСписать = Количество Тогда
				Сумма = ВыборкаПартия.СуммаОстаток;
			Иначе
				Сумма = Количество * ВыборкаПартия.СуммаОстаток / ВыборкаПартия.КоличествоОстаток;
			КонецЕсли;
